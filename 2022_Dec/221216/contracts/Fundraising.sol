...
// 와디즈 크라우드 펀딩 서비스 앱 같은 경우도 이런식으로 
// 핵심 기능만 뽑아서 간단하게 작성한것
contract Fundraising {
    ...
    // weeks이 단위도 이더리움 가상 머신에서 제공하는 단위
    ...
    receive() external payable{
        ...
        // 도네이션 
        // 누가 얼마를 후원했는지 맵핑 객체에 value를 더해준다.
    ...
    function withdrawDonations() external {
        ...
        // 총액이 목표 금액과 같거나 큰지 확인
        require(reisedAmount >= targetAmount);
        // 데드라인 시간이 남았는지 확인
        require(block.timestamp > finishTime);

        // 펀딩이 기간이 마감되었을때 주최자한테 모금된 금액을 풀어주는 기능
        payable(owner).transfer(reisedAmount);
    }

    // 펀딩 마감 되고 목표액에 도달 못했을때 환불 해주는 함수
