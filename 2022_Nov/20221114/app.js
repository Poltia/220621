...
// 최소한의 정보로 인증하기위해 블록체인을 전부 다운로드 하지않고
// 머클 루트값만 가진다. 

// SHA256알고리즘으로 256bit로 구성된 64자리 문자열로 암호화 해준다 했고.
// 머클루트는 값은 64자 이고
// 안전하고 다양한 검증을 빠르게 할수 있는 것
// 머클 트리 자체가 해시로 이루어져 있고 하나의 트랜잭션이나
// 아니면 블록의 값이 변조 되면 머클루트 해시 값이 번조되고
...
// 비트코인 지갑 생성
// Random 값을 만들고 -> 개인키와 공개키 생성(타원 곡선 알고리즘 사용 ECC) -> 지갑 주소 생성(SHA258&Bace58)

// 블록체인의 동작
// 브로드 캐스팅(알려준다 내가 새로운 거래를 만들었다라면)
// 1. 지갑 프로그램에 연결된 peerlist를 확인하고 
// 2. 네트워크 브로드 캐스팅
// 3. 채굴 시작(내가 먼저 다른 사람이 만든 거래를 승인(블록 생성)을 해주고 수수료를 받아야지)
// 4. 채굴 성공 (내가 찾은 nonce를 모두에게 알린다.)
// 5. 채굴에 성공한 블록을 전달
// 6. 네트워크 브로드 캐스팅
// 7. 블록을 검증을 한후 블록 체인에 연결
// 8. 롱기스트 블록 체인

// P2P기술에서 P peer의 약자
// P2P는 서비스에서 모두가 서버가 될수있고 클라이언트가 될수 있다.
...
// { 주소 : 누구, 코인 : 49, 참조 트랜잭션 : Tx 0001} 객체가 만들어 지고
// 이 두 객체를 통해서 만들어진 내용이 UTXO에 객체로 담기게 된다 (미사용 객체)
// {주소: 누구, 얻은 코인 : 50, 참조 트랜잭션 : Tx 0000} 사용된 객체는 제거
...
// 새로 생긴 unspentTxOut 객체들을 UTXO에 추가 시켜주는 작업을 하고
// UTXO 내용을 업데이트 해준다. 

// 트랜잭션 풀 이라는 공간을 만들고 트랜잭션 객체가 생성됬을때 만들어진 트랜잭션을 
...
// 노드들이 트랜잭션 풀의 내용을 공유하는 부분은 트랜잭션 객체가 생겼을때 트랜잭션 객체를 
// 브로드캐스트 하는 방식

// 윈도우에 geth를 설치해서 쓰면 더 쉬운데 맥 사용자들이 있어서
// 예전엔 윈도우를 Linux를 쓰려면 가상머신을 설치해서 써야했는데

// 컴퓨터에 메모리나 CPU 등등 자원을 쪼개서 OS를 따로 설치해서 사용할수 있는데
// 이러면 컴퓨터 성능이 줄어들어 비효율 적인 단점이 있다.
// WSL은 이 단점을 보완하고 개발 환경을 쉽게 구축 가능

// WSL2를 설치해서 사용할거다
// 윈도우에서 Linux 명령어를 사용할수 있게 도와주는 툴(WSL2)

// 제어판 -> 프로그램 -> 프로그램 및 기능 -> windows 기능 켜기/끄기
// "Linux용 windows 하위 시스템" or "windows Subsystem for Linux" 이걸 체크

// 윈도우 키 + S 검색창에 설정 검색 -> 시스템 정보 -> 정보 -> 윈도우 버전 확인
// cmd 창을 관리자 권한으로 실행 
