...
EVM은?
이더리움 가상머신(Ethereum Virtual Machin)의 약자 (JAVA에서 JVM같은 개념적인 비슷한 기능)
스마트 건트랙트를 실행할 때 분산 네트워크 환경에서 모든 노드들이 스마트 컨트랙트에 의해 특정한 결과를 얻어야할 때
...
EOA로 트랜잭션을 시작할 수 있다.
EOA는 다른 EOA 또는 CA에 메시지를 모낼 수 있으며, 이 동작은 개인키를 사용해서 트랜잭션을 만들고 서명하고
...
RPC는 원격 프로시저 호출해 별도의 원격 제어를 위한 코딩없이 다른 주소 공간에서 함수나 프로시저를 실행할 수 있게하는 프로세스간의 통신기술.
