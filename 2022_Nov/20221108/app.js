...
/*
머클 트리란
머클 트리는 수많은 트랜잭션을 각각 해싱해서
2개의 짝을 지어 해싱을 계속 반복해서 최종적으로 하나가 남을 때 까지 해싱한 트리형태의 구조
1. 첫번째 데이터를 SHA256형태의 해시값으로 변환
2. 2개의 노드를 한쌍으로 묶어서 해시값 변환
3. 해시값으로 변환을 반복해서 마지막 하나가 남을때까지 반복
4. 블록의 모든 거래를 합친 해시값이 머클루트
머클 트리가 필요한 이유는 머클 트리의 목적이 _데이터의 간편성과 확실한 인증_이기 때문이다.

블록체인 네트워크 정보를 모두 다운로드 하면 무척 용량이 크기 때문에
최소한의 정보로 인증하기 위해 블록체인을 전부 다운로드 하지 않고 머클루트 값만 가진다.

SHA256 알고리즘으로 256bit로 구성된 64자리 문자열로 암호화 했기 때문에 머클루트 값은 64자 이고
안전하고 다양한 검증을 빠르게 할 수 있다.
머클트리 자체가 해시로 이루어져 있고 하나의 트랜잭션이나 블록의 값이 변조되면 머클루트 해시값이 바뀌고
...
비트코인 지갑 생성
Random 값을 만들고 -> 개인키와 공개키 생성(타원곡선 알고리즘 사용 ECC) -> 지갑 주소 생성(SHA256 & Base58)

블록체인의 동작
broadcating(내가 새로운 거래를 만들었다면 알려준다.)
1. 지갑 프로그램에 연결된 peerlist를 확인
2. 네트워크 브로드캐스팅
3. 채굴 시작(내가 먼저 다른사람이 만든 거래를 승인(블록생성)해주고 수수료를 받는다)
4. 채굴 성공(내가 찾은 nonce를 모두에게 알린다)
5. 채굴에 성공한 블록을 전달
6. 네트워크 브로드캐스팅
7. 블록을 검증한 후 블록체인에 연결
8. 롱기스트 블록체인

    (P2P기술에서 P는 peer의 약자
    P2P는 서비스에서 모두가 서버가 될 수 있고, 클라이언트가 될 수 있다.
    P2P는 서로 다른 사용자들끼리 자료를 주고받는다라는 뜻
    Peer는 동등한 사용자를 의미)

비트코인 화폐를 발행하는 방식이 마이닝이고
비트코인 화폐는 10분에 한번씩 일정량이 생성되며
마이닝에 참여한 사용자 중 한명에게 지급된다.
참여자들은 hashchash라는 문제를 풀고(hashcash는 특정한 조건을 가지는 해시값을 찾는 것)
*/